...
AJSH_Rotate_GizmoY::AJSH_Rotate_GizmoY()
{
	...
	ConstructorHelpers::FObjectFinder<UMaterial> OriginMaterial(TEXT("/Script/Engine.Material'/Game/JSH/BP/Gizmo/MM_Rotate_Green.MM_Rotate_Green'"));
	...
	ConstructorHelpers::FObjectFinder<UMaterial> RedMaterialLoader(TEXT("/Script/Engine.Material'/Game/JSH/BP/Gizmo/MM_Rotate_Green.MM_Rotate_Green'"));
	if (RedMaterialLoader.Succeeded())
	{
		GreenMaterial = RedMaterialLoader.Object;
	}
	
	Tags.Add(FName("Rotate_Gizmo_Y"));
}
...
void AJSH_Rotate_GizmoY::NotifyActorOnClicked(FKey ButtonPressed)
{
	Super::NotifyActorOnClicked(ButtonPressed);

}

void AJSH_Rotate_GizmoY::GOnClicked()
{
	...
	if (Clicked)
	{
		...
		FRotator DeltaRotation = FRotator(End_variation, 0.0f, 0.0f);
	...
	//
	// 	End_Rotate = FRotator(Start_Rotate.Pitch  + End_variation, Start_Rotate.Yaw, Start_Rotate.Roll);
	//
	// 	OriginPlayer->Editor_SpawnActor->Set_Rotate_from_Gizmo(End_Rotate);
	// }
}
...
// 오버랩 색상 변경
void AJSH_Rotate_GizmoY::BeginCursorOver()
{
...
void AJSH_Rotate_GizmoY::EndCursorOver()
{
...
void AJSH_Rotate_GizmoY::OriginColor()
{
	...
	if (GreenMaterial)
	{
		...
		Origin2->SetMaterial(0, GreenMaterial);
		Origin3->SetMaterial(0, GreenMaterial);
		Origin4->SetMaterial(0, GreenMaterial);
	}
}

void AJSH_Rotate_GizmoY::SelectedColor()
{
	if (YellowMaterial)
	{
...
void AJSH_Rotate_GizmoY::HandleMouseReleaseOutsideActor()
{
	Clicked = false;
	firstclick = false;
	SelectedGizmo = false;
...
void AJSH_Rotate_GizmoY::Visible_and_Collision_On()
{
...
void AJSH_Rotate_GizmoY::Visible_and_Collision_Off()
{
	Origin->SetVisibility(false);
	Origin->SetCollisionProfileName(TEXT("NoCollision"));
	Origin2->SetVisibility(false);
...
void AJSH_Rotate_GizmoY::BeginPlayer(AJSH_Player* temp, AJSH_PlayerController* control)
{
	OriginPlayer = temp;
	if (OriginPlayer)
	{
		OriginPlayer->Save_Gizmo_RY(this);
	}
