...
// Sets default values
AJSH_Scale_GizmoBox::AJSH_Scale_GizmoBox()
{
	...
	ConstructorHelpers::FObjectFinder<UStaticMesh> TMesh(TEXT("/Script/Engine.StaticMesh'/Game/JSH/Material/TransformGizmo/Meshes/SM_UniformScaleHandle.SM_UniformScaleHandle'"));
	if (TMesh.Succeeded())
	{
	...
	Tags.Add(FName("Scale_Gizmo_Box"));
}


// Called when the game starts or when spawned
void AJSH_Scale_GizmoBox::BeginPlay()
{
	Super::BeginPlay();
}

// Called every frame
void AJSH_Scale_GizmoBox::Tick(float DeltaTime)
{
...
void AJSH_Scale_GizmoBox::NotifyActorOnClicked(FKey ButtonPressed)
{
	Super::NotifyActorOnClicked(ButtonPressed);

}

void AJSH_Scale_GizmoBox::GOnClicked()
{
	...
	if (OriginPlayer->Editor_SpawnActor->GizmoY_ON || OriginPlayer->Editor_SpawnActor->GizmoZ_ON || OriginPlayer->Editor_SpawnActor->GizmoX_ON) return;
	...
	if (Clicked)
	{
		...
		End_variation = (EndMouselocation_2D_Y - StartMouselocation_2D_Y) * -0.01;
		
		End_Scale = FVector(Start_Scale.X + End_variation, Start_Scale.Y + End_variation, Start_Scale.Z + End_variation);

		// -값으로 가면 3d object 앞뒤가 뒤집혀 버어림
		//End_Scale = FVector(FMath::Abs(Start_Scale.X + ((End_Location.X - StartMouselocation.X) * 0.01)), FMath::Abs(Start_Scale.Y + ((End_Location.Y - StartMouselocation.Y) * 0.01)), FMath::Abs(Start_Scale.Z + ((End_Location.Z - StartMouselocation.Z) * 0.01)));
...
// 오버랩 색상 변경
void AJSH_Scale_GizmoBox::BeginCursorOver()
{
...
void AJSH_Scale_GizmoBox::EndCursorOver()
{
...
void AJSH_Scale_GizmoBox::OriginColor()
{
...
void AJSH_Scale_GizmoBox::SelectedColor()
{
	if (YellowMaterial)
	{
...
void AJSH_Scale_GizmoBox::HandleMouseReleaseOutsideActor()
{
	Clicked = false;
	firstclick = false;
	SelectedGizmo = false;
...
void AJSH_Scale_GizmoBox::Visible_and_Collision_On()
{
	Origin->SetVisibility(true);
	Origin->SetCollisionProfileName(TEXT("Gizmo"));
}
void AJSH_Scale_GizmoBox::Visible_and_Collision_Off()
{
	Origin->SetVisibility(false);
	Origin->SetCollisionProfileName(TEXT("NoCollision"));
}

void AJSH_Scale_GizmoBox::BeginPlayer(AJSH_Player* temp, AJSH_PlayerController* control)
{
	OriginPlayer = temp;
	if (OriginPlayer)
	{
		OriginPlayer->Save_Gizmo_SB(this);
	}
